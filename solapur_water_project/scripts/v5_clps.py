"""
v5_clps.py  ·  place in: scripts/
V5 - Composite Leak & Pressure Score (CLPS)
Phase 3.5 FINAL — generates alerts per scenario (baseline/leak/valve/surge).
...
Usage: python scripts/v5_clps.py
"""
...
def main():
    ...
    for scen_name, (p_file, f_file) in scenarios.items():
        ...
        for z in zones:
            ...
            if z in ["zone_1", "1"]:
                rp_s = f_df[[p for p in res_pipes if p in f_df.columns]].abs().sum(axis=1)
                rp_b = f_base[[p for p in res_pipes if p in f_base.columns]].abs().sum(axis=1)
                f_in   = f_in   + rp_s
                f_in_b = f_in_b + rp_b
        ...
        pd.DataFrame(clps_rows).to_csv(
            os.path.join(OUT, f"clps_scores_{scen_name}.csv"), index=False
        )
