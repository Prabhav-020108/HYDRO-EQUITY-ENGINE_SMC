...
# # ══════════════════════════════════════════════════════════════════════
# #  TRIGGER B — Leak Rules (→ engineer_recs only)
# #  Uses 'leak' scenario alerts from V5. Falls back to 'baseline'.
# # ══════════════════════════════════════════════════════════════════════

# def trigger_b_leak(alerts_all, influence, conn):
#     """Writes to engineer_recs based on dominant CLPS signal."""
#     # Prefer 'leak' scenario, fall back to 'baseline'
#     alerts = alerts_all.get('leak', alerts_all.get('baseline', []))
#     if isinstance(alerts_all, list):
#         alerts = alerts_all
...
#             'vid':    f"V-{zone_id.upper().replace('_','')}",
#             'pid':    pipe_id,
#             'dp':     round(-clps * 12, 2),  # estimated pressure loss
#             'urg':    urgency,
...
#             # Trigger A — Equity (all 4 channels)
...
#             # Trigger B — Leak (engineer only)
#             bc = trigger_b_leak(alerts_all, influence, conn)
#             eng_total += bc
#             print(f'  [B] Leak    → {bc} engineer_recs')
...
#         print('  Make sure db_setup_phase4b.py has been run first.')
#         return
