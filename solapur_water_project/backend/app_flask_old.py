...
What's in this file:
  ...
  - Fullcity WNTR CSV files used (pressure_fullcity_*.csv)
  - /zones  → plain list from v4_zone_status.json
  - /alerts/active → per-scenario, deduped, with db_alert_id for resolution
  - /burst-risk/top10 → lat/lon normalised for map markers
  - Pipeline zone-filter for performance (?zone=zone_5)
  - Resolution workflow: acknowledge → field-action → resolve (PostgreSQL)
...
app = Flask(__name__)
CORS(app)  # Open to all origins — needed for VS Code Live Server (5500) → Flask (5000)
...
def _load_real_data():
    ...
    if _real_data:
        print(f'[app] ★  REAL WNTR DATA ACTIVE: {list(_real_data.keys())}')
    else:
        print('[app] ⚠  No real data — run simulation_engine.py all')

_load_real_data()

# ── Formula fallback ─────────────────────────────────────────
ESR_SOURCES = [
    {'lat': 17.6935, 'lon': 75.8810, 'head': 72},
    {'lat': 17.7012, 'lon': 75.9280, 'head': 68},
    {'lat': 17.6623, 'lon': 75.9034, 'head': 65},
    {'lat': 17.6878, 'lon': 75.8654, 'head': 70},
    {'lat': 17.6758, 'lon': 75.9412, 'head': 60},
    {'lat': 17.7080, 'lon': 75.8950, 'head': 66},
    {'lat': 17.6700, 'lon': 75.8780, 'head': 63},
]
...
def _formula_pressure(lat, lon, scenario='baseline', hour=8):
    ...
    for s in ESR_SOURCES:
        ...
        if d < md:
            md = d; sh = s['head']
    p = sh - md*3.8 + (lat-17.655)*22*0.4 + (lon-75.960)*15*0.4 - _dmult(hour)*3
    p += ((abs(lat*1000)*7 + abs(lon*100)*13) % 100) / 100.0 * 6 - 3
    if scenario == 'leak':
...
def get_pressure(lat, lon, scenario='baseline', hour=8):
    if scenario in _real_data and _nodes_df is not None:
        try:
            df = _real_data[scenario]
            t  = _hour_to_timestep(hour)
            d  = ((_nodes_df['lat'] - lat)**2 + (_nodes_df['lon'] - lon)**2)
            ci = d.idxmin()
            row = df.iloc[t]
            for name in [f'J{ci}'] + [f'J{ci+o}' for o in [1,-1,2,-2,5,-5]]:
                if name in row.index:
                    return round(max(0, min(100, float(row[name]))), 2)
        except Exception:
            pass
...
def pipeline():
    ...
    ?zone=all      → all pipes (cached by frontend on first load)
    ?zone=zone_5   → only Zone 5 pipes (zone-filter for performance)
    """
...
def infrastructure():
    ...
    if not items:
        items = [{'lat': e['lat'], 'lon': e['lon'], 'type': 'ESR', 'name': f'ESR-{i+1}', 'zone': ''}
                 for i, e in enumerate(ESR_SOURCES)]
        ...
        with open(path, newline='', encoding='utf-8') as f:
            for i, row in enumerate(csv.DictReader(f)):
                ...
                try:
                    ...
                    nodes.append({'lat': lat, 'lon': lon, 'pressure': p, 'color': _pressure_color(p)})
                except Exception: pass
...
def equity():
    ...
    if os.path.exists(path):
        with open(path, 'r', encoding='utf-8') as f:
            try:
                data  = json.load(f)
                return jsonify({'zones': data.get('zones', []),
...
@app.route('/zones')
def zones_endpoint():
    """Returns v4_zone_status.json as a plain list for the frontend."""
...
def simulate():
    ...
    if scen in _real_data and _nodes_df is not None:
        ...
        d           = ((_nodes_df['lat'] - z['lat'])**2 + (_nodes_df['lon'] - z['lon'])**2)
        ci          = d.idxmin()
        node_name   = f'J{ci}'
        tl = []
        for h in range(24):
...
def data_status():
    ...
    return jsonify({'real_data_scenarios': list(_real_data.keys()),
...
def alerts():
    ...
    if os.path.exists(v5_path):
        with open(v5_path, 'r', encoding='utf-8') as f:
            try:
                data = json.load(f)
                al   = data.get(scen, []) if isinstance(data, dict) else data
...
def alerts_active():
    ...
    Includes db_alert_id for resolution workflow buttons.
    Baseline: sidebar only (no map markers).
    Leak/Valve/Surge: sidebar + map markers.
    """
    scen    = request.args.get('scenario', 'baseline')
    v5_path = op('v5_alerts.json')
    ...
    al = data.get(scen, data.get('baseline', [])) if isinstance(data, dict) else data
    ...
    data = {
        ...
        'leak': [
            {'zone': 'Zone 3 · Zone 5 — URGENT',
             'action': 'Dispatch field team to (17.682, 75.942). Isolate pipe segment east of ESR-5.',
        ...
        'valve': [
            {'zone': 'Zone 8 — CRITICAL',
             'action': 'Reopen south distribution valve immediately. Activate tanker supply for Ward 14 & 15.',
        ...
        'surge': [
            {'zone': 'Zone 1 · Zone 6 — HIGH',
             'action': 'Increase ESR outlet pressure by 12–15%. Throttle Zone 3 inlet by 18%.',
             'impact': 'HEI improvement: 0.64 → 0.82 est.'},
            {'zone': 'System-wide — ADVISORY',
             'action': 'Stagger supply: Zone 1–4 at 06:00, Zone 5–8 at 07:30.',
...
def burst_risk():
    ...
    if os.path.exists(path):
        with open(path, 'r', encoding='utf-8') as f:
            try:
                data = json.load(f); data['data_source'] = 'v6_engine'
                return jsonify(data)
            except Exception: pass
...
def burst_risk_top10():
    """Normalises lat/lon from start_lat/start_lon so map markers work."""
    ...
    if os.path.exists(path):
        with open(path, 'r', encoding='utf-8') as f:
            try:
                ...
                return jsonify({'segments': segs, 'count': len(segs), 'data_source': 'v6_engine'})
            except Exception as e:
    ...
    try:
        with engine.connect() as conn:
            result = conn.execute(text("""
                UPDATE alerts SET status='acknowledged', acknowledged_at=NOW(), notes=:notes
                WHERE alert_id=:aid AND status='fired'
                RETURNING alert_id, zone_id
            """), {'aid': alert_id, 'notes': notes})
            row = result.fetchone(); conn.commit()
        if row:
            with engine.connect() as conn:
                conn.execute(text("""
...
def field_action_alert(alert_id):
    ...
    try:
        with engine.connect() as conn:
            result = conn.execute(text("""
                UPDATE alerts SET status='field_action', field_action_at=NOW(), notes=:notes
        ...
        return jsonify({'error': 'Must be acknowledged before field action'}), 400
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def resolve_alert(alert_id):
    ...
    try:
        with engine.connect() as conn:
            result = conn.execute(text("""
                UPDATE alerts SET status='resolved', resolved_at=NOW(), notes=:notes
        ...
        return jsonify({'error': 'Alert not found or not in resolvable state'}), 404
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def alerts_lifecycle():
    try:
        with engine.connect() as conn:
            result = conn.execute(text("""
                SELECT alert_id, zone_id, clps, severity, dominant_signal, scenario,
                       status, created_at, acknowledged_at, field_action_at, resolved_at, notes
...
if __name__ == '__main__':
    print('=' * 55)
    print('  Hydro-Equity Engine Backend v3.5 · SMC')
