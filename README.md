...
## Current State of the Project (Phase 1)

Phase 1 is complete. The following is exactly what exists and works right now:

- The full SMC pipeline GeoJSON has been parsed, cleaned, and structured into CSVs
- A WNTR hydraulic simulation runs on the real Zone 1 network and produces pressure/flow CSVs
- A Flask backend serves all data to the dashboard via REST endpoints
- A single-file Leaflet dashboard shows the real pipe network with pressure colouring, scenario switching, alerts, and recommendations
...
│   ├── water_source.geojson          # Water source areas (MultiPolygon, 3 features)
│   ├── storage_tank.geojson          # ESRs and storage tanks (63 features)
│   ├── raw_station.geojson           # Pumping stations (3 features)
│   ├── pipe_segments.csv             # 10,160 cleaned pipes with all analysis columns
│   ├── nodes_with_elevation.csv      # 14,085 junction nodes with elevation
│   ├── zone_demand.csv               # Demand per zone (8 rows)
│   └── infrastructure_points.csv     # All infrastructure combined (66 rows)
│
├── scripts/
│   ├── v1_data_foundation.py         # V1: parses GeoJSON → produces all Data/ CSVs
│   └── simulation_engine.py          # V3: WNTR model → pressure/flow CSVs + .inp file
│
├── backend/
│   └── app.py                        # Flask API serving all endpoints on port 5000
│
├── frontend/
│   └── index.html                    # Complete single-file dashboard (no build step)
│
├── outputs/                          # Generated by simulation_engine.py — not committed
│   ├── pressure_baseline.csv
│   ├── flow_baseline.csv
│   ├── pressure_scenario_A_leak.csv
│   ├── pressure_scenario_B_valve_close.csv
...
│   └── solapur_network.inp
│
├── DATA_CONTRACT.md                  # Every column, unit, and assumption documented
└── requirements.txt
```
...
Phase 1 ends with this pipeline working end to end:

```
GeoJSON → cleaned CSVs → WNTR simulation → Flask API → Leaflet dashboard
```

The next work to build on top of this:

**Connect real WNTR outputs to analytics (V4, V5, V6):**
- `pressure_baseline.csv` has pressure per node per timestep. V4 needs to read this, identify tail-end nodes per zone using the NetworkX DiGraph that `simulation_engine.py` already builds, and compute actual HEI per zone per timestep. This replaces the formula-approximated equity values in the dashboard.
- `flow_baseline.csv` has flow per pipe per timestep. V5 can compute the four CLPS signals (PDR, FPI, NFA, DDI) from this directly.
- `pipe_segments.csv` has `assumed_age_years`, `design_lifespan_years`, material. Combined with node pressures to approximate pipe pressure, V6 PSS can run on every pipe segment.

**Replace Flask with FastAPI (V2 proper):**
Required for the async scenario simulation endpoint. The scenario panel sends a POST, the simulation runs in background, the frontend polls for the result. This is not possible synchronously because WNTR takes 5–15 seconds per run.

**Build scenario simulation panel (V8):**
Interactive sliders for valve reduction %, ESR pressure adjustment, supply window. Calls the async endpoint. Shows before/after HEI per zone on the map.

Read `DATA_CONTRACT.md` before building on V1 outputs — it documents every column, unit, and assumption.
