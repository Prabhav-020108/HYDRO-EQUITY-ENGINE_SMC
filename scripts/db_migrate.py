...
Migrates all V1/V4/V5/V6 CSV and JSON outputs into PostgreSQL.
Run after db_setup.py. Safe to re-run (clears and reloads).
"""
...
def migrate_alerts():
    ...
    with open(path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    # v5_alerts.json may be a list or dict keyed by scenario
    ...
    with engine.connect() as conn:
        ...
        for a in all_alerts:
            ...
            """), {
                ...
                'clps':           float(a.get('clps', a.get('CLPS', 0)) or 0),
    ...
    with engine.connect() as conn:
        ...
        for s in segments:
            ...
            """), {
                ...
                'lat_start':      float(s.get('lat_start', s.get('lat', 0)) or 0),
                'lon_start':      float(s.get('lon_start', s.get('lon', 0)) or 0),
                'lat_end':        float(s.get('lat_end', 0) or 0),
                'lon_end':        float(s.get('lon_end', 0) or 0),
            })
        conn.commit()
